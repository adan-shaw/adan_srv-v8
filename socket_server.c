...
static int reserve_id (struct socket_server *ss)
{
	...
	{
		id = __sync_add_and_fetch (&(ss->alloc_id), 1);
		if (id < 0)
		{
...
struct socket_server *socket_server_create ()
{
	...
	struct socket *s;
	poll_fd efd = ev_create ();
	if (ev_invalid (efd))
	{
		fprintf (stderr, "socket-server: create event pool failed.\n");
		return NULL;
	}
	//udp 通过创建pipe 管道到epoll 中, 来实现io 时间监听
	if (pipe (fd))
	{
		ev_release (efd);
...
{
	struct write_buffer *tmp, *wb = list->head;
	while (wb)
	{
		tmp = wb;
		wb = wb->next;
...
static struct socket *new_fd (struct socket_server *ss, int id, int fd, int protocol, uintptr_t opaque, bool add)
{
...
static int open_socket (struct socket_server *ss, struct request_open *request, struct socket_message *result)
{
	...
	int status, sock = -1, id = request->id;
	...
	if (status == 0)
	{
		...
		addr = ai_ptr->ai_addr;
		sin_addr = (ai_ptr->ai_family == AF_INET) ? (void *) &((struct sockaddr_in *) addr)->sin_addr : (void *) &((struct sockaddr_in6 *) addr)->sin6_addr;
	...
	else
	{
		...
		ev_write (ss->event_fd, ns->fd, ns, true);
	}
...
static int send_list_udp (struct socket_server *ss, struct socket *s, struct wb_list *list, struct socket_message *result)
{
	...
	{
		tmp = list->head;
		sasz = udp_socket_address (s, tmp->udp_address, &sa);
		err = sendto (s->fd, tmp->ptr, tmp->sz, 0, &sa.s, sasz);
		if (err < 0)
		{
			switch (errno)
			{
...
static int send_list (struct socket_server *ss, struct socket *s, struct wb_list *list, struct socket_message *result)
{
...
static int send_buffer (struct socket_server *ss, struct socket *s, struct socket_message *result)
{
	...
	if (s->high.head == NULL)
	{
		...
		{
			// step 4
			ev_write (ss->event_fd, s->fd, s, false);
...
static struct write_buffer *append_sendbuffer_ (struct socket_server *ss, struct wb_list *s, struct request_send *request, int size, int n)
{
...
static inline void append_sendbuffer (struct socket_server *ss, struct socket *s, struct request_send *request, int n)
{
...
static int send_socket (struct socket_server *ss, struct request_send *request, struct socket_message *result, int priority, const uint8_t * udp_address)
{
	...
	if (s->type == SOCKET_TYPE_INVALID || s->id != id || s->type == SOCKET_TYPE_HALFCLOSE || s->type == SOCKET_TYPE_PACCEPT)
	{
	...
	if (send_buffer_empty (s) && s->type == SOCKET_TYPE_CONNECTED)
	{
		...
		else
		{
			...
			sasz = udp_socket_address (s, udp_address, &sa);
		...
		ev_write (ss->event_fd, s->fd, s, true);
	}
	else
	{
		if (s->protocol == PROTOCOL_TCP)
...
{
	int id = request->id, listen_fd = request->fd;
...
static int bind_socket (struct socket_server *ss, struct request_bind *request, struct socket_message *result)
{
	...
	result->opaque = request->opaque;
	result->ud = 0;
	s = new_fd (ss, id, request->fd, PROTOCOL_TCP, request->opaque, true);
	if (s == NULL)
	{
		result->data = NULL;
		return SOCKET_ERROR;
...
static void add_udp_socket (struct socket_server *ss, struct request_udp *udp)
{
	...
	ns = new_fd (ss, id, udp->fd, protocol, udp->opaque, true);
	if (ns == NULL)
	{
		close (udp->fd);
		ss->slot[HASH_ID (id)].type = SOCKET_TYPE_INVALID;
...
static int set_udp_address (struct socket_server *ss, struct request_setudp *request, struct socket_message *result)
{
	...
	if (s->type == SOCKET_TYPE_INVALID || s->id != request->id)
	{
		return -1;
	}
	type = request->address[0];
	if (type != s->protocol)
	{
		// protocol mismatch
		result->opaque = s->opaque;
...
static int forward_message_tcp (struct socket_server *ss, struct socket *s, struct socket_message *result)
{
	int sz = s->p.size;
...
static int forward_message_udp (struct socket_server *ss, struct socket *s, struct socket_message *result)
{
	union sockaddr_all sa;
	uint8_t *data;
...
static int report_connect (struct socket_server *ss, struct socket *s, struct socket_message *result)
{
	int error, code;
	void *sin_addr;
	...
	socklen_t slen, len;
	len = sizeof (error);
	code = getsockopt (s->fd, SOL_SOCKET, SO_ERROR, &error, &len);
	if (code < 0 || error)
	{
		force_close (ss, s, result);
		return SOCKET_ERROR;
...
static int report_accept (struct socket_server *ss, struct socket *s, struct socket_message *result)
{
	...
	ev_nonblocking (client_fd);
	ns = new_fd (ss, id, client_fd, PROTOCOL_TCP, s->opaque, false);
	if (ns == NULL)
	{
		close (client_fd);
		return 0;
...
int socket_server_poll (struct socket_server *ss, struct socket_message *result, int *more)
{
	...
	for (;;)
	{
		...
		{
			ss->event_n = ev_wait (ss->event_fd, ss->ev, MAX_EVENT);
			ss->checkctrl = 1;
			if (more)
			{
				*more = 0;
...
static void free_buffer (struct socket_server *ss, const void *buffer, int sz)
{
	struct send_object so;
...
{
	int listen_fd, family = 0;
	listen_fd = do_bind (host, port, IPPROTO_TCP, &family);
	if (listen_fd < 0)
	{
		return -1;
	}
...
int64_t socket_server_udp_send (struct socket_server *ss, int id, const struct socket_udp_address *addr, const void *buffer, int sz)
{
	int addrsz;
...
int socket_server_udp_connect (struct socket_server *ss, int id, const char *addr, int port)
{
	...
	addrsz = gen_udp_address (protocol, (union sockaddr_all *) ai_list->ai_addr, request.u.set_udp.address);
