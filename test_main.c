...
static void test (void)
{
	...
	c = socket_server_connect (ss, 100, "127.0.0.1", 8888);	//connect 非阻塞test
	printf ("connecting %d\n", c);
...
void *my_srv_poll (void *pval)
{
	struct socket_message result;
	struct socket_server *ss = pval;
	int type, id = socket_server_listen (ss, 200, "127.0.0.1", 8888, 32);	//监听
	printf ("socket_server listening, id=%d\n", id);
	socket_server_start (ss, 201, id);	//启动srv
	...
	for (;;)
	{
		...
		case SOCKET_CLOSE:
			printf ("close(%lu) [id=%d]\n", result.opaque, result.id);
			//socket_server_close(ss,result.opaque,result.id);//踢close() 的id 出epoll
			break;
		case SOCKET_OPEN:
			printf ("open(%lu) [id=%d] %s\n", result.opaque, result.id, result.data);
			break;
		case SOCKET_ERROR:
			printf ("error(%lu) [id=%d]\n", result.opaque, result.id);
			//socket_server_close(ss,result.opaque,result.id);//踢error() 的id 出epoll
			break;
		case SOCKET_ACCEPT:
			printf ("accept(%lu) [id=%d %s] from [%d]\n", result.opaque, result.ud, result.data, result.id);
			break;
	...
	socket_server_exit (ss);			//退出srv
	printf ("my_srv_poll() quit\n");
}

void *my_cli (void *pval)
{
	void *pdata;
	int i, id_pool[test_count];
	struct socket_server *ss = pval;
	...
	for (i = 0; i < test_count; i++)
	{
		socket_server_close (ss, 400 + i, id_pool[i]);	//踢error() 的id 出epoll
	}
...
{
	pthread_t pid_s, pid_c;
	struct socket_server *ss = socket_server_create ();
	pthread_create (&pid_s, NULL, my_srv_poll, ss);	//创建新线程for srv io_event (仅只处理io事件, 为做任何收发io 操作)
	sleep (1);
	pthread_create (&pid_c, NULL, my_cli, ss);
